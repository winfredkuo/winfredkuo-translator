...
app.add_middleware(
    ...
    allow_origins=[
        "http://localhost:8000",
        "http://127.0.0.1:8000",
        "https://winfredkuo-translator.vercel.app",
    ],
...
TRANSLATOR_API_BASE_URL = os.environ.get(
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box} body{margin:0;min-height:100vh;font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:radial-gradient(circle at top,#12304e 0,#07111f 42%,#040a12 100%);color:var(--text)}
    .wrap{max-width:980px;margin:0 auto;padding:24px 16px 40px}.hero{padding:24px 0 18px}.kicker{letter-spacing:.24em;text-transform:uppercase;color:var(--accent);font-size:.76rem}.title{font-size:clamp(2rem,5vw,4.5rem);line-height:.98;margin:.3rem 0 .8rem}.sub{color:var(--muted);max-width:64ch;line-height:1.7}.grid{display:grid;gap:16px;grid-template-columns:repeat(2,minmax(0,1fr))}@media (max-width:860px){.grid{grid-template-columns:1fr}}
    ...
    .label{font-size:.82rem;color:var(--muted);margin-bottom:8px;text-transform:uppercase;letter-spacing:.14em}.controls{display:flex;gap:10px;flex-wrap:wrap;align-items:center}.select,.input,.btn{border-radius:16px;border:1px solid rgba(86,170,208,.35);background:rgba(6,15,28,.9);color:var(--text)}.select,.input{padding:12px 14px;font-size:1rem}.input{width:100%;min-height:120px;resize:vertical}.btn{padding:12px 16px;font-weight:700;cursor:pointer}.primary{background:linear-gradient(135deg,var(--accent),var(--accent2));color:#031019;border:none}.ghost{background:rgba(12,25,40,.8)}.danger{color:var(--danger)}.panel{min-height:170px;white-space:pre-wrap;line-height:1.8;font-size:1.08rem}.toprow{display:flex;gap:12px;align-items:center;justify-content:space-between;flex-wrap:wrap}.badge{padding:8px 12px;border-radius:999px;background:rgba(66,215,255,.12);color:#bfefff;border:1px solid rgba(66,215,255,.18)}
    .tiny{font-size:.85rem;color:var(--muted);line-height:1.6}.stack{display:grid;gap:10px}.actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:12px}.status{margin-top:10px;color:#b6c8d8;min-height:1.4em}.divider{height:1px;background:linear-gradient(90deg,transparent,var(--line),transparent);margin:14px 0}
  </style>
</head>
<body>
  <main class="wrap">
    <section class="hero">
      <div class="kicker">Smart Translator</div>
      <h1 class="title">旅行時可直接開口說，立刻翻成對方聽得懂的語言。</h1>
  ...
  <script>
    const BASE = "${TRANSLATOR_API_BASE_URL}";
...
def translate(request: Request):
    try:
        ...
        body = {}
    # FastAPI Request.json() is async; in Vercel runtime this handler is rarely used.
    # Keep a small fallback route so the app remains importable.
    return JSONResponse({"detail": "Use the Cloudflare Worker endpoint from the browser app."}, status_code=501)
